"""
Platform Registrations: registration-list
"""
from app0.admin.db import Query, SearchResults, db
from app0.admin.services.registration_services import get_registrations
...
async def run(payload: Query, context: EventContext) -> SearchResults:
    es = db(context.env)
    results = await get_registrations(es, payload)
