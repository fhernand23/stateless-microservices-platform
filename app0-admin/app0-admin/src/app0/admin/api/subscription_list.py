"""
Platform Subscriptions: subscription-list
"""
...
from app0.admin.services.subscription_services import get_subscriptions
...
async def run(payload: Query, context: EventContext) -> SearchResults:
    es = db(context.env)
    results = await get_subscriptions(es, payload)
